...
- **Web terminal clipboard is wired explicitly, not left to xterm defaults.** xterm.js renders selection as a painted overlay (no DOM Selection) and, on Linux/Windows, turns `Ctrl+C`→`\x03` and `Ctrl+V`→`\x16` — and that `\x16` reaches the *remote* agent, which pastes the *server's* clipboard. So `TerminalPane.tsx` owns clipboard: **copy-on-select** (copy in the `mouseup`/`touchend` user gesture so `copyToClipboard`'s `execCommand` fallback is permitted over plain-HTTP; gated by the `ui.copy_on_select` preference), a **right-click `ContextMenu`** driven in controlled mode from our own `onContextMenu` (Copy / Paste / Select all — never `ContextMenuTrigger`, whose hardcoded 500ms touch long-press fights the terminal's own long-press-to-select), and chords `Ctrl+Shift+C` + `Ctrl+Insert` + `Cmd+C`. Paste is `Ctrl+V` (intercepted via `attachCustomKeyEventHandler` returning `false` so xterm emits no `\x16` and the browser's native `paste` event feeds xterm's handler — secure-context-free), plus `Ctrl+Shift+V` / `Cmd+V`; context-menu Paste uses `navigator.clipboard.readText`, guarded because it throws synchronously off a secure context. **Classify clipboard chords by physical key (`code`/`keyCode`), never `ev.key`** — xterm decides `\x16` positionally by `keyCode`, so a `key`-based match silently misses on non-Latin layouts and re-opens the remote-clipboard leak (`classifyClipboardKey` in `lib/termkeys.ts`). `Ctrl+C` stays SIGINT; on Mac a lone `Control` passes through to the app. Keep this when touching `TerminalPane.tsx`.
...
- **Restart brings tabs back dormant, not running.** On reopen each tab that comes up alone for its provider may resume (per the per-provider dynamic-liveness rule) — reopening a dormant tab resumes that provider's conversation when it is the sole live tab of that provider; each other tab reopens as a pill with **no process spawned**, and its content surface shows a **provider-agnostic** message (dux doesn't restore a tab's conversation across a restart; recover the prior conversation from a fresh session using the provider's own conversation-history command — do not name a specific command, providers differ). An **explicit action** (Enter in the TUI, a "Start fresh session" button in the web) launches a fresh session with no continuation; **focus alone never launches**. On the web this means a dormant tab must render from a `has_live_process` flag *without* opening the PTY socket, because subscribing force-launches the provider.
