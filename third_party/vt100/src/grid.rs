...
impl Grid {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            // dux patch: capture into scrollback whenever the scroll region is
            // top-anchored (scroll_top == 0), matching xterm/alacritty/wezterm. The
            // upstream guard (!scroll_region_active()) drops history for any bottom
            // margin, which kills scrollback for agent TUIs that pin an input bar.
