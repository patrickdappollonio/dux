...
import { applyModifiers, arrowSeq, classifyClipboardKey, ESC, TAB } from "@/lib/termkeys"
...
export function TerminalPane({ kind, id, sessionId }: TerminalPaneProps) {
  ...
  useEffect(() => {
    ...
    // Clipboard key chords. xterm's defaults don't bridge the browser clipboard
    // on Linux/Windows: Ctrl+V emits \x16 to the REMOTE agent (pasting the
    // server's clipboard) and Ctrl+C / a selection never reach the system
    // clipboard. We intercept only the clipboard chords; everything else (Ctrl+C
    // SIGINT, plain typing, mac Control/Cmd) passes through to xterm unchanged.
    // `isMac` is stable for this mount.
    const isMac = isApplePlatform()
    term.attachCustomKeyEventHandler((e) => {
      if (e.type !== "keydown") return true
      const action = classifyClipboardKey({
        ctrlKey: e.ctrlKey,
        shiftKey: e.shiftKey,
        altKey: e.altKey,
        metaKey: e.metaKey,
